...
function makeEdgeId(from: string, to: string) { return `${from}--${to}` }
...
const INITIAL: MapData = {
  ...
  edges: [
    { id: 'node-0--node-1', from: 'node-0', to: 'node-1' },
    { id: 'node-0--node-2', from: 'node-0', to: 'node-2' },
    { id: 'node-1--node-3', from: 'node-1', to: 'node-3' },
    { id: 'node-2--node-3', from: 'node-2', to: 'node-3' },
    { id: 'node-3--node-4', from: 'node-3', to: 'node-4' },
    { id: 'node-3--node-5', from: 'node-3', to: 'node-5' },
    { id: 'node-4--node-6', from: 'node-4', to: 'node-6' },
    { id: 'node-5--node-6', from: 'node-5', to: 'node-6' },
  ],
}
...
export default function MapEditorScreen() {
  ...
  const handleSVGClick = useCallback((e: React.MouseEvent) => {
    ...
    if (tool === 'delete') {
      if (hit) {
        ...
        // delete edge — find closest edge
        const closest = findClosestEdge(map, x, y)
        if (closest) {
          setMap(m => ({ ...m, edges: m.edges.filter(e => e.id !== closest) }))
        }
      }
      return
    }
    ...
    if (tool === 'connect') {
      ...
      if (!connecting) {
        ...
        if (connecting !== hit.id) {
          const edgeId = makeEdgeId(connecting, hit.id)
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    setMap(m => ({
      ...m,
      nodes: m.nodes.map(n => n.id === dragging ? { ...n, x: nx, y: ny } : n),
    }))
  }, [dragging, dragOffset])
  ...
  const applyLabel = () => {
    ...
    setMap(m => ({
      ...m,
      nodes: m.nodes.map(n => n.id === editingId ? { ...n, label: editLabel } : n),
    }))
  }
  ...
  const changeType = (type: NodeType) => {
    ...
    setMap(m => ({
      ...m,
      nodes: m.nodes.map(n => n.id === editingId ? { ...n, type } : n),
    }))
  }
  ...
  return (
    <div style={{ display: 'flex', height: '100%', background: '#1a1208', color: '#e8d5b0', fontFamily: 'Georgia, serif' }}>
      ...
      {/* PANEL BOCZNY */}
      <div style={{ width: 200, padding: 12, borderRight: '1px solid #3a2510', display: 'flex', flexDirection: 'column', gap: 12, flexShrink: 0 }}>
        <div style={{ fontSize: 14, fontWeight: 'bold', color: '#c8902a' }}>🗺️ Map Editor</div>

        {/* Narzędzia */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          <div style={{ fontSize: 10, color: '#6a5040', textTransform: 'uppercase', marginBottom: 2 }}>Narzędzie</div>
        ...
        {tool === 'add' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            <div style={{ fontSize: 10, color: '#6a5040', textTransform: 'uppercase', marginBottom: 2 }}>Typ węzła</div>
            {NODE_TYPES.map(t => (
              <button key={t} onClick={() => setAddType(t)} style={{
                padding: '4px 8px', textAlign: 'left', fontSize: 11,
        ...
        {editingId && selectedNode && tool === 'select' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, borderTop: '1px solid #3a2510', paddingTop: 8 }}>
            ...
            />
            <div style={{ fontSize: 10, color: '#6a5040' }}>Typ:</div>
            {NODE_TYPES.map(t => (
        ...
        {/* Status połączenia */}
        {tool === 'connect' && (
          <div style={{ fontSize: 11, color: connecting ? '#c8902a' : '#6a5040', background: '#1e1208', padding: 8, borderRadius: 4, border: '1px solid #3a2510' }}>
            {connecting
              ? `Kliknij drugi węzeł aby połączyć`
              : `Kliknij pierwszy węzeł`}
          </div>
        )}
        ...
        {/* Akcje */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          <button onClick={exportJSON} style={{ padding: '6px 8px', fontSize: 11, background: '#0a2a1a', border: '1px solid #2a6040', color: '#70c890', cursor: 'pointer', borderRadius: 4 }}>
            ⬇ Eksportuj JSON
          </button>
          <button onClick={resetMap} style={{ padding: '6px 8px', fontSize: 11, background: '#2a0a0a', border: '1px solid #6a2020', color: '#c07070', cursor: 'pointer', borderRadius: 4 }}>
            ↺ Reset mapy
          </button>
        </div>
          ...
          {map.edges.map(edge => {
            ...
            return (
              <g key={edge.id}>
                ...
                />
                {/* Strzałka kierunku */}
                <circle cx={mx} cy={my} r={3} fill="#7a6030" />
              </g>
            )
          })}
              ...
              >
                ...
                {(isSelected || isConnecting) && (
                  <circle
                    ...
                    r={NODE_RADIUS + 6}
                    fill="none"
                    stroke={isConnecting ? '#c8902a' : '#d4a830'}
                    strokeWidth={2}
                    opacity={0.6}
                  />
                )}
