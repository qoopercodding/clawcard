...
export function FrameConfigTest() {
  ...
  async function runTests() {
    ...
    // ── Test 4: POST dry run — prawdziwy payload ─────────────────────────
    try {
      const payload = {
        ...
        isNew: false, // nie modyfikuje card.types.ts
      }
      const res = await fetch('/api/save-frame-config', {
        method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(payload),
      })
      if (!res.ok) { r.push({ name: 'POST /api dry run', status: 'fail', detail: `HTTP ${res.status}` }); }
      else {
    ...
    // ── Test 5: Store — localStorage pending_card_type ───────────────────
    // Testujemy bezpośrednio przez localStorage (synchroniczne) — nie przez React state
    try {
      ...
      r.push({
        name: 'Store: pending_card_type handoff (localStorage)',
        status: readBack?.typeName === '__e2e_pending__' && !stillThere ? 'ok' : 'fail',
        detail: readBack ? `Zapisano: '${readBack.typeName}' | Usunięto: ${!stillThere}` : 'Zapis localStorage nie powiódł się',
      })
    } catch (e) {
      r.push({ name: 'Store: handoff', status: 'fail', detail: String(e) })
    }

    // ── Test 6: Store — user_cards localStorage ──────────────────────────
    // Testujemy przez localStorage bezpośrednio
    try {
      ...
      r.push({
        ...
        detail: found ? 'Karta zapisana i odczytana z localStorage ✓' : 'Karta nie znaleziona w localStorage',
      })
    } catch (e) {
      r.push({ name: 'Store: addCard', status: 'fail', detail: String(e) })
    }
    ...
    r.push({
      ...
      detail: `${allCards.length} kart łącznie (${Object.values(userCards).length} użytkownika + ${allCards.length - Object.values(userCards).length} sample)`,
    })
  ...
  return (
    <div style={{padding:'24px', maxWidth:900, margin:'0 auto', overflowY:'auto', height:'calc(100vh - 76px)'}}>
      ...
      <button onClick={runTests} disabled={running}
        style={{padding:'10px 20px', borderRadius:10, border:'1px solid rgba(134,239,172,0.4)',
          background:'rgba(5,40,20,0.7)', color:'#86efac', fontSize:'0.9rem', fontWeight:700,
      ...
      {Object.values(userCards).length === 0 ? (
        <p style={{color:'#4b5563', fontSize:'0.82rem'}}>Brak kart użytkownika. Utwórz kartę w Card Editorze i kliknij "🃏 Dodaj do galerii".</p>
      ) : (
